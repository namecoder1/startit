...
import { TbSettings, TbListSearch } from "react-icons/tb";


const navbar = async () => {
	const session = await auth()
	return (
		<header className='rounded-full drop-shadow-xl bg-white border-[1px] border-black/10 p-3 mx-3 mt-4 sm:w-fit sm:mx-auto'>
					...
					<>
						...
						<Popover>
							<PopoverTrigger className='flex items-center justify-center gap-2'>
								<Image src={`${session?.user.image}`} alt='Immagine utente' width={25} height={25} className='rounded-full border-[1px] drop-shadow-sm' />
							...
							<PopoverContent className='w-fit mt-1'>
								<div className='flex items-start justify-center flex-col p-1 gap-y-2'>
									...
									<Link href='/discover' className='flex sm:hidden items-center justify-center gap-2'><TbListSearch />Discover</Link>
